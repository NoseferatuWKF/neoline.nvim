...
M.file_path = function(opts)
  opts = opts or {}
  return el_sub.buf_autocmd("el_file_path", "BufRead",
    wrap_fnc(opts, function(_, buffer)
      if not M.try_devicons() then return "" end
      local fmt = opts.fmt or "%s"
      local ext = vim.fn.fnamemodify(buffer.name, ":p:e")
      local file, hl = M._devicons.get_icon(buffer.name, ext:lower(), { default = true })
      if file then
        if opts.hl_icon then
          local hlgroup = M.extract_hl({
            bg = { StatusLine = "bg" },
          ...
          file = set_hl(hlgroup, buffer.name)
        end
